function [accuracy, Ball, predictions, prob, stat] = TestTrainMNR(featureChoice)
...
for g = 1:length(files)
    [sl, sr, ss] = snipEEG(files{g},Tsnip,after,factor);
    featl = featEEGsnip(sl,factor,featureChoice);
    featr = featEEGsnip(sr,factor,featureChoice);
    feats = featEEGsnip(ss,factor,featureChoice);
...
[Ball,~,stat] = mnrfit(XTrain, yTrain', "Interactions","on");
...
%% Function definitions:
    function features = featEEGsnip(snips, factor, featureChoice)
        Fs = 5000/factor;
        ...
        for i = 1:seconds
            ...
            deltaPower(i, 1) = bandpower(snips(i,:,1), Fs, deltaBand);
            deltaPower(i, 2) = bandpower(snips(i,:,2), Fs, deltaBand);
        ...
        switch featureChoice
            ...
            case '3'
                features = [alphaPower, betaPower,deltaPower];
            case '4'
                features = [alphaPower, betaPower,deltaPower,thetaPower];
            case '5'
                features = [alphaPower, thetaPower];
            case '6'
                features = [alphaPower, deltaPower,thetaPower];
            case '7'
                features = [alphaPower, deltaPower];
        end
